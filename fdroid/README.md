...
1. Update `Builds[0].commit` to the actual commit/tag being submitted for initial
   review if it's no longer `v1.0.255` / `39dbe2f`.
...
3. Install `fdroidserver` and run `fdroid readmeta`, `fdroid lint`, and
   `fdroid build --verbose com.rrajath.grove:255` against a local checkout of
