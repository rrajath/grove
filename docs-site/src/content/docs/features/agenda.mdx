...
Settings → Agenda → Widget controls the home-screen Agenda ledger widget (not the [quick-capture widget](/features/widget)), with a live preview above the controls: background Transparency, how many Days ahead to show, per-row Filename/Tags/Priority toggles, and a Small/Medium/Large Font size. These are independent of the in-app Levers panel above — changing how the widget looks never changes the Agenda screen. A shown filename contracts each nested folder to its first letter (`Work/Projects/notes.org` → `W/P/notes.org`); the file's own name stays whole.
