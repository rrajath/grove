...
A plain tap on `[[ ]]` inserts `[[link][description]]` with `link` selected. **Long-pressing** it opens a menu with three link builders:
...
- **File or heading** opens a drill-down picker (the same one used for refiling) to choose any heading in the vault. A heading in the note you're editing becomes `[[*Heading]]`; one in another file becomes `[[file:other.org::*Heading]]`. If the heading has an `ID` or `CUSTOM_ID`, Grove offers to use that instead — an id link keeps working after the heading is renamed or moved.
- **Heading by ID** does the same, but only lists headings that have an `ID` or `CUSTOM_ID`, and always writes the id link.
