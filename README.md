...
- **Real editor**: raw org subtree editing with syntax highlighting, a formatting toolbar (long-press the link button to build a URL link or link to any heading in the vault, by name or by `ID`/`CUSTOM_ID`), list continuation on Enter, a metadata sheet (TODO state, priority, tags with autocomplete, SCHEDULED/DEADLINE), repeater advancement on DONE, and autosave with a stale-file guard.
