...
class OrgMutationsTest {
    ...
    // --- refreshHeadingCookie ---

    @Test
    fun `heading cookie counts done vs total TODO subheadings`() {
        ...
        val update = OrgMutations.refreshHeadingCookie(doc, doc.headlines.first())!!
        assertEquals("* TODO Ship it [2/3]", update.text.lines()[0])
        assertTrue(!update.complete)
    }

    @Test
    fun `heading cookie falls back to its own checklist when it has no TODO subheadings`() {
        val doc = OrgParser.parse(
            """
            * TODO Ship it [/]
            - [ ] write code
    ...
    @Test
    fun `heading with TODO subheadings prefers them over a checklist in its own body`() {
        val doc = OrgParser.parse(
            """
            * TODO Ship it [/]
            - [ ] an unrelated checklist item
            ** DONE Write code
            ** DONE Write tests
            """.trimIndent() + "\n"
        )
        val update = OrgMutations.refreshHeadingCookie(doc, doc.headlines.first())!!
        // Both subheadings done: 2/2, not the 0/1 the stray checklist item would give.
        assertEquals("* TODO Ship it [2/2]", update.text.lines()[0])
        assertTrue(update.complete)
    }

    @Test
    fun `complete is true once every counted child is done`() {
        val doc = OrgParser.parse(
            """
            * TODO Ship it [/]
            ** DONE Write code
    ...
    @Test
    fun `heading percent cookie recalculates as a rounded percentage`() {
        val doc = OrgParser.parse(
            """
            * TODO Ship it [%]
            ** DONE Write code
        ...
        val update = OrgMutations.refreshHeadingCookie(doc, doc.headlines.first())!!
        assertEquals("* TODO Ship it [50%]", update.text.lines()[0])
    }

    @Test
    fun `heading with no cookie in its title is left alone`() {
        val doc = OrgParser.parse(
            """
            * TODO Ship it
            ** DONE Write code
        ...
        assertNull(OrgMutations.refreshHeadingCookie(doc, doc.headlines.first()))
    }

    @Test
    fun `heading cookie with nothing to count returns null`() {
        val doc = OrgParser.parse("* TODO Ship it [/]\nplain body text, no checklist\n")
        assertNull(OrgMutations.refreshHeadingCookie(doc, doc.headlines.first()))
    }

    @Test
    fun `only direct child headlines count, not grandchildren`() {
        val doc = OrgParser.parse(
            """
            * TODO Ship it [/]
            ** DONE Backend
        ...
        val update = OrgMutations.refreshHeadingCookie(doc, doc.headlines.first())!!
        // Two direct children (Backend, Frontend), one done — the nested
        // "Migration" grandchild under Backend isn't folded into the count.
        assertEquals("* TODO Ship it [1/2]", update.text.lines()[0])
    }

    @Test
    fun `checklist fallback only counts the heading's own body, not a subheading's`() {
        val doc = OrgParser.parse(
            """
            * TODO Ship it [/]
            - [X] own item
            ** Notes
            - [ ] a checklist item that belongs to the Notes subheading
            """.trimIndent() + "\n"
        )
        val update = OrgMutations.refreshHeadingCookie(doc, doc.headlines.first())!!
        assertEquals("* TODO Ship it [1/1]", update.text.lines()[0])
    }
}
