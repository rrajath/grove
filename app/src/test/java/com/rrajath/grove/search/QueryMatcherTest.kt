...
class QueryMatcherTest {
    ...
    @Test
    fun `agenda splits overdue into its own section from the day buckets`() {
        ...
        assertEquals(listOf("Overdue"), agenda.overdue.map { it.title })
        assertEquals(2, agenda.days.size)
        assertEquals(today, agenda.days[0].date)
        assertEquals(listOf("Today"), agenda.days[0].notes.map { it.title })
        assertEquals(today.plusDays(1), agenda.days[1].date)
        assertEquals(listOf("Thursday"), agenda.days[1].notes.map { it.title })
    }
    ...
    fun `overdue sorted oldest first then priority then title`() {
        ...
        val tiedNoPriority = note("TiedNone", scheduled = "<2025-06-01 Sun>")
        val doneOld = note("DoneOld", scheduled = "<2025-05-01 Thu>", done = true)

        val agenda = QueryMatcher.agenda(listOf(recentB, tiedA, tiedB, tiedNoPriority, doneOld), 1, today)
        assertEquals(listOf("TiedA", "TiedB", "TiedNone", "RecentB"), agenda.overdue.map { it.title })
    }

    @Test
    fun `day bucket sorted by priority then time then title`() {
        val noTimeHighPriority = note("Alpha", scheduled = "<2025-06-11 Wed>", priority = "A")
        val timed9 = note("Bravo", scheduled = "<2025-06-11 Wed 09:00>", priority = "A")
        val timed14 = note("Charlie", deadline = "<2025-06-11 Wed 14:00>", priority = "A")
        val lowPriorityEarly = note("Delta", scheduled = "<2025-06-11 Wed 08:00>", priority = "B")

        val agenda = QueryMatcher.agenda(listOf(noTimeHighPriority, timed9, timed14, lowPriorityEarly), 1, today)
        assertEquals(1, agenda.days.size)
        assertEquals(
            listOf("Bravo", "Charlie", "Alpha", "Delta"),
            agenda.days[0].notes.map { it.title },
        )
    }

    @Test
    fun `growing the day window extends future days without reprocessing overdue`() {
        ...
        val small = QueryMatcher.agenda(listOf(overdue, soon, later), 3, today)
        assertEquals(listOf("Overdue"), small.overdue.map { it.title })
        assertEquals(1, small.days.size)
        assertEquals(listOf("Soon"), small.days[0].notes.map { it.title })

        val grown = QueryMatcher.agenda(listOf(overdue, soon, later), 10, today)
        assertEquals(listOf("Overdue"), grown.overdue.map { it.title })
        assertEquals(2, grown.days.size)
        assertEquals(listOf("Soon", "Later"), grown.days.map { it.notes.single().title })
    }
