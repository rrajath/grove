...
- **Org link resolution**: tapping a link in Read mode routes through `DocumentViewModel.openOrgLink`. `OrgLinkParser` (pure, JVM-tested) classifies the target between `[[ ]]` — `id:`, `#custom-id`, `*Heading`, `file:` / bare-path (with an optional `::*heading` / `::#id` search option, resolved relative to the linking file's directory), an external scheme, or an unqualified "fuzzy" string. The ViewModel then resolves it against the live `OrgDocument`, the Room note index (`notes.orgId` / `notes.customId` — how cross-file `id:` and `#custom-id` links land without opening every file), and `Vault.open` for a cross-file heading match. An `id:` target and a `#custom-id` target resolve to a heading anywhere in the vault; a `*Heading` and a no-scheme `[[Heading]]` target only ever match a heading title in the current file. All of these open Read mode of the matched heading (`onOpenNote`). A whole-file target opens that file's outline (`onOpenOutline`); a `file:x.org::#id` / `::*heading` target opens that heading's Read mode, falling back to the file's outline if only the in-file heading is missing; an external scheme is handed to the OS; anything unresolved shows a `GroveToast`. The **other direction** — building a link — is the note editor toolbar's link button (`ui/editor/LinkEditing.kt`, pure + JVM-tested, its output round-tripped back through `OrgLinkParser`): a tap inserts a `https://` URL scaffold (or the neutral `[[link][description]]` template when nothing is selected), a long-press opens a file / heading picker in the `RefileSheet` drill-down. A file picked at its top level emits `[[file:rel/path.org]]`; a heading emits a bare `[[*Heading]]` / `[[#custom-id]]` / `[[id:uuid]]` for the current file or a `file:`-qualified form (path relative to the editing file) for another. When the picked file carries a file-level `:ID:`, or the picked heading an `:ID:` / `:CUSTOM_ID:`, a dialog offers the resilient id link in place of the plain one.
