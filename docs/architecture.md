...
- **Read mode**: a custom `AnnotatedString` renderer over the org AST (`BlockParser`/`InlineTokenizer`); the note subtree renders in a `LazyColumn` (one item per child heading) so long notes compose lazily, with document traversals memoized per document. Tables render as monospace plain text in v1. File-level `#+` keywords (document preamble) and `:PROPERTIES:` drawers render as faded, collapsible monospace key-value sections (`CollapsibleKvSection`), gated by the "Show header tags" / "Show property drawers" Settings toggles — display-only; the `.org` file is never rewritten.
