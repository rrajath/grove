...
- **"NEW" feature badges** (`ui/newbadge/`): a small config-driven framework for pointing returning users at a newly-shipped feature. Each feature is one `NewFeature(id, since, anchors, destination)` in the `NEW_FEATURES` registry (`NewFeatures.kt`); `since` is the `versionCode` it shipped in, `anchors` are string keys for every navigational element that should carry the badge, `destination` is the one whose viewing retires it. `NewBadgeState.from(GroveSettings)` derives the live set: a feature is active when a baseline is recorded, `since > baseline`, and the id isn't in `seenNewFeatures`. The baseline (`GroveSettings.newBadgeBaseline`, DataStore key `new_badge_baseline`) is seeded once — to `lastSeenChangelogBuild ?: currentBuild` — so a fresh install lands on the current build and shows nothing, while an update carries its older build and badges anything newer. State reaches the UI through `LocalNewBadges` (provided at `GroveNavigation`); a call site renders `NewBadge(anchorKey)` (a filled accent `Pill`, nothing when inactive) and the destination site adds `MarkNewFeatureSeen(anchorKey)`, which writes every matching feature id into `seenNewFeatures` when that screen *leaves* composition (so a badge on the destination itself stays visible for the whole visit) — clearing the badge from the whole trail at once. `newBadgeBaseline` / `seenNewFeatures` are device-specific and excluded from settings export. Debug builds expose Settings › Developer › "Reset New badges" (`resetNewBadges()`: clears the seen set, drops the baseline to 0).
