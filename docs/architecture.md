...
Supporting pieces: `OrgTimestamp` (parse/format org timestamps incl. repeaters), `OrgKeywords` (configurable TODO/DONE keyword sets, `"TODO IN-PROGRESS | DONE CANCELLED"` syntax), `InlineTokenizer` and `BlockParser` (inline markup and block structure for rendering), and `LineEditing` (pure cursor-aware helpers for list continuation, heading demotion, and new-note caret placement, shared by both editors).

## The vault (`vault/`)

`FileStore` is a small suspend interface (`list / read / write / create / rename / delete / exists / pruneEmptyDirs`) over a **tree** of files. Every path is vault-relative with `/` separators (`projects/clients/acme.org`); a root-level file's path is just its name. Two implementations:
...
`pruneEmptyDirs(dir)` removes `dir` and any now-empty ancestor directories (stopping at the first non-empty one, never touching the vault root), collapsing empty descendant subdirectories first. `Vault` calls it after a delete or folder move empties a directory.

`Vault` is the facade the rest of the app uses: it lists notebooks recursively (filtering non-`.org` files, Syncthing conflict copies, and `.orgzlyignore` matches), opens files as parsed `OrgDocument`s with an `(name, mtime, size)`-keyed parse cache, and implements create (into any directory, creating missing folders), rename, `moveNotebook` (relocate keeping the file name), `renameFolder` / `deleteFolder` (rename or delete a whole directory by moving/deleting every descendant `.org` file, then pruning the emptied folder), save, and delete. "Delete" is **permanent** — SAF exposes no OS-level trash, so `deleteNotebook` / `deleteFolder` remove the file(s) outright via `DocumentsContract.deleteDocument` and prune the emptied directory. Recovery relies on the sync backend's own history (Syncthing file versioning).
...
`SearchViewModel` no longer holds the whole vault in memory. It keeps only a `NoteFacetRow` projection (no titles, no bodies) to build the filter catalog, the blank-state quick counts, and the file-name match pass above, and loads full rows per search, scoped to the candidates. The Agenda screen and the ledger widget likewise read only rows that actually have a SCHEDULED or DEADLINE, through the body-less `PlannedNoteRow` projection (`toNoteMeta` maps it to `NoteMeta` exactly as it does a full `NoteEntity`); the planning-dates calendar reads a still-narrower `plannedTimestamps()` — just the distinct date strings. `NoteMeta` lazily caches its parsed scheduled/deadline/closed/created dates; sorting and the agenda view use those instead of re-running the timestamp regex per comparison.
...
Two Glance (`androidx.glance.appwidget`, 1.2.0) widgets, both RemoteViews under
the hood so they render outside the app's own process/lifecycle. Glance is held
at 1.2.0 or newer deliberately: 1.1.1 had a defect where an app with more than
one widget could cross-wire their generated layouts under update churn and render
one widget's content in the other's slot (the Capture widget redrawing as the
Agenda ledger, or the reverse).

- **Capture** (`CaptureWidget`/`CaptureWidgetReceiver`): one tap, opens the
  capture picker via the `grove://capture` deep link. No state of its own. The
  asterisk mark is `Image(ImageProvider(R.drawable.ic_brand_asterisk))`, a
  vector reproducing `BrandMark.drawAsterisk`'s five-spoke geometry — not a
  `"✱"` text glyph, which U+2731 renders as blank/tofu on devices whose system
  font lacks it.
...
- **Navigation**: a single `NavHost` (`Routes`) with `ModalNavigationDrawer`; deep links for `grove://note/{id}` and `grove://capture`. A note is addressed by `NoteRef` = `"path/to/file.org@headlineLineIndex"` (optionally `#customId`; the file part is the vault-relative path; `NoteRef.decode` splits on the last `@`, and `Routes.encode` percent-encodes the `/`s), so a ref names any heading in any file. A file's preamble (everything before its first heading) has no headline to key off of. It splits into three independently editable regions: the file-level property drawer, the **preface** (the `#+KEY:` lines) and the **intro** (the free text between them and the first heading). The preface **editor** gets its own route, `preface/{fileName}` (`Routes.preface`). The intro's **read view** rides the normal `note/{id}` route with a sentinel `NoteRef.lineIndex == INTRO_LINE_INDEX` (`-1`, `NoteRef.isIntro` / `NoteRef.intro(file)`), which also flows unchanged through search results and the index (one FTS row per intro, keyed at `-1`); its editor is rendered in place of that route's edit mode rather than getting a route of its own. A single metadata **drawer** (file-level `:PROPERTIES:`, or a heading's `:PROPERTIES:` / `:LOGBOOK:`) is editable in isolation through `drawer/{fileName}?kind={kind}&noteId={noteId}` (`Routes.drawer`), which renders the same editor screen. A single `#+BEGIN_x … #+END_x` **block** in a note body is likewise editable through `block/{fileName}?line={line}` (`Routes.block`), keyed by the block's doc line.
- **Configuration changes**: `.MainActivity` declares a full `android:configChanges` set (orientation, screen size, `uiMode`, `density`, `fontScale`, `locale`, …), so a rotation or a system dark-mode / font-scale change never recreates the Activity. Compose re-reads `LocalConfiguration` / `LocalDensity` and re-lays-out in place, which keeps every screen's in-progress state (an unsaved editor buffer, a half-filled capture, an in-progress date edit, scroll positions) alive across the change. This is safe only because the app is Compose-only with a DataStore-driven theme — nothing reads a `Configuration` field outside Compose or relies on recreation to apply a change. It does **not** cover process death; editor/capture buffers are not yet persisted to `SavedStateHandle` (see `internal/rotation-state-loss-audit-2026-09-01.md`).
- **Org link resolution**: tapping a link in Read mode routes through `DocumentViewModel.openOrgLink`. `OrgLinkParser` (pure, JVM-tested) classifies the target between `[[ ]]` — `id:`, `#custom-id`, `*Heading`, `file:` / bare-path (with an optional `::*heading` / `::#id` search option, resolved relative to the linking file's directory), an external scheme, or an unqualified "fuzzy" string. The ViewModel then resolves it against the live `OrgDocument`, the Room index (`notes.orgId` / `notes.customId` for a heading, `notebooks.orgId` for a file-level `:ID:` — all three columns indexed, so cross-file `id:` / `#custom-id` links land in O(log n) without opening every file), and `Vault.open` for a cross-file heading match. An `id:` target resolves to a heading anywhere in the vault, or — when it matches a file's leading-property-drawer `:ID:` (`OrgDocument.fileId`, or `notebooks.orgId` across files) — to that file's outline. A `#custom-id` target resolves to a heading anywhere in the vault; a `*Heading` and a no-scheme `[[Heading]]` target only ever match a heading title in the current file. A matched heading opens Read mode (`onOpenNote`). A whole-file target opens that file's outline (`onOpenOutline`); a `file:x.org::#id` / `::*heading` target opens that heading's Read mode, falling back to the file's outline if only the in-file heading is missing; an external scheme is handed to the OS; anything unresolved shows a `GroveToast`. The **other direction** — building a link — is the note editor toolbar's link button (`ui/editor/LinkEditing.kt`, pure + JVM-tested, its output round-tripped back through `OrgLinkParser`): with text selected and a URL already on the clipboard, a tap inserts the finished `[[<clipboard-url>][<selection>]]` with nothing to type (the Emacs `org-insert-link` behaviour of picking a URL out of the kill ring; `applyToolbarLink` in `LinkClipboard.kt` reads the clipboard only when text is selected, so the OS paste toast fires only on a real link action); otherwise a tap inserts a `https://` URL scaffold (or the neutral `[[link][description]]` template when nothing is selected), and a long-press opens a file / heading picker in the `RefileSheet` drill-down. A file picked at its top level emits `[[file:rel/path.org]]`; a heading emits a bare `[[*Heading]]` / `[[#custom-id]]` / `[[id:uuid]]` for the current file or a `file:`-qualified form (path relative to the editing file) for another. When the picked file carries a file-level `:ID:`, or the picked heading an `:ID:` / `:CUSTOM_ID:`, a dialog offers the resilient id link in place of the plain one.
- **Deep-link intent as a one-shot**: `grove://` links (shortcuts, widget, notification) arrive as `MainActivity`'s launch Intent (cold start) or via `onNewIntent` (warm). `MainActivity` threads it to `GroveApp` as Compose state; `GroveNavigation` hands it to `NavController.handleDeepLink` once and then calls `onDeepLinkConsumed(intent)`, which nulls the state and `setIntent(Intent())`. The launch Intent is only read when `savedInstanceState == null`. Together this stops a recreation (process death, or an uncovered config change) from re-dispatching a deep link the user already dismissed — e.g. reopening a cancelled capture sheet. The callback carries the exact Intent and is ignored unless it is still current, so an `onNewIntent` racing an in-flight resolve isn't lost. The `grove://capture` destination is a `ModalBottomSheet`; its `onDismiss` pops the back stack, and a fast double back-press could fire that callback twice (once from the sheet, once from the system back dispatcher). On the widget's shallow deep-link back stack that popped past the start destination and left the `NavHost` with nothing to render (a blank screen), so `onDismiss` is guarded by `NavBackStackEntry.isResumed()` — a popped entry drops below `RESUMED`, so the second callback no-ops.
...
- **Editor**: a `BasicTextField` with `OrgVisualTransformation`, highlight-only spans with `OffsetMapping.Identity`, so the text is never altered and cursor math stays trivial. Highlighting is memoized per line content, so a keystroke re-tokenizes only the edited line rather than the whole buffer. `EditNoteScreen` edits one note's subtree, not the whole file; saving splices the subtree back via `OrgMutations.replaceSubtree`, guarded by a revision check (stale-file banner → Overwrite or Reload). A FAB-created note opens with a blank body line appended and the caret placed per Settings § Notes' **New note cursor** lever (`GroveSettings.newNoteCursor`): on that body line (`BODY`, the default) or just past the heading's `* ` (`HEADING`, via `LineEditing.newNoteCaret`, which inserts the space if the stars lack one). `EditRegionScreen` is the same pattern scoped to one region of a file instead of a subtree (`EditorViewModel.loadRegion` / `EditorUiState.region`): a smaller sibling with no heading, so no Read/Edit toggle or metadata sheet. Every region is a line range spliced back with `OrgMutations.replaceLines`, recomputed from the fresh file on save: `INTRO` via `OrgMutations.introRange`, `PREFACE` via `prefaceRange`, the three drawer regions (`FILE_PROPERTIES`, `HEADING_PROPERTIES`, `HEADING_LOGBOOK`) via `fileDrawerRange` / `headingDrawerRange`, and `BLOCK` (a `#+BEGIN_x … #+END_x` body block, markers and folded affiliated lines included — or a standalone `#+KEYWORD:` run) via `blockRange`. The ranges are disjoint and exclude trailing blank lines, so editing one region never disturbs another or the blank line separating them. It opens via double-tapping the matching `CollapsibleKvSection` / `CollapsibleLogSection` / `CollapsibleBlockSection` (`doubleTapToEdit`, same gesture and timing as double-tapping any other note text into edit mode). `EditPrefaceScreen` and `EditIntroScreen` are thin wrappers for their regions.

  **Auto-save.** Every editor saves 5 s after typing stops. Settings § Notes' **Auto-save notes** toggle (`GroveSettings.autoSaveNotes`, DataStore key `auto_save_notes`, default on) switches that idle timer off in `EditNoteScreen` and `EditRegionScreen` alike, leaving the top-bar save icon and the "Save changes?" leave dialog (`UnsavedNoteDialog`) as the only paths to disk. The capture editor's own 5 s idle write is unaffected: it persists a recoverable draft, not a note. Switching Read ⇄ Edit never writes either way — see Read mode's pending buffer below.
- **Read mode**: a custom `AnnotatedString` renderer over the org AST (`BlockParser`/`InlineTokenizer`); the note subtree renders in a `LazyColumn` (one item per visible child heading) so long notes compose lazily, with document traversals memoized per document and body lines resolved per row. A note whose subtree exceeds `LARGE_SUBTREE_THRESHOLD` (60 headings) opens with its inner headings folded (`defaultReadCollapse` seeds every descendant that has children into the collapsed set), so only the note body + a one-level section list mount; smaller notes open fully expanded. Each foldable heading carries an outline-style disclosure caret; `visibleReadRows` filters out anything nested under a folded ancestor. Text selection is scoped per section (one `SelectionContainer` per rendered heading), the trade-off for letting the subtree virtualize instead of living in one eager `Column`. Org tables render as a grid (`OrgTableView`, fed by the JVM-tested `parseOrgTable`): a pinned, bolded header row (every row above the first `|---|` rule), one shared horizontal scroll so wide tables pan with the header tracking columns, and a body capped at 55% of screen height that scrolls vertically beneath the pinned header; the editor still shows the raw pipe text. File-level `#+` keywords (document preamble), `:PROPERTIES:` drawers and `:LOGBOOK:` drawers render as faded, collapsible monospace sections (`CollapsibleKvSection` / `CollapsibleLogSection`), gated by the "Show preface" / "Show property drawers" Settings toggles. `#+BEGIN_x … #+END_x` body blocks (`BlockParser.OrgBlock.Block`) render the same way (`CollapsibleBlockSection`), headed by the block kind with a `#+BEGIN_SRC` language shown alongside, but open expanded and are always shown. A run of standalone `#+KEYWORD:` lines with no block below it (any `#+word:` that isn't a `#+BEGIN_`/`#+END_` marker: `#+CAPTION:`, `#+NAME:`, `#+ATTR_*`, …) also renders through `CollapsibleBlockSection` as an `OrgBlock.Block` with `keywordRun = true` — labelled `KEYWORDS` for two or more consecutive lines, else the sole keyword's name — but opens collapsed like the metadata drawers; a blank line ends a run, and a run directly above a `#+BEGIN` still folds into that block instead. They never rewrite the `.org` file on their own, but double-tapping anywhere on a body row opens `EditRegionScreen` scoped to that region (`onDoubleTap`): the `#+` keyword run for PREFACE, the single drawer for a `:PROPERTIES:` / `:LOGBOOK:` section, or the whole block (or standalone keyword run) for a `Block` section. The **intro** (real content before the first heading, or the whole body of a file with no headings — an org-roam note) is surfaced separately: the Outline shows a tap-only `IntroRow` above the headings, opening an intro-only Read view (`IntroContent`, reusing `BodyBlocks`; filename-only breadcrumb; no title or subtree), whose Read/Edit toggle opens the intro-only editor. The intro is counted as a note and full-text indexed. Its metadata sheet opens with `headline = null`; a state/priority/tag/date/note chip first calls `DocumentViewModel.withIntroHeading`, which splices a blank `* ` heading above the content (`OrgMutations.wrapIntroInHeading`) in one atomic edit, then applies the change and re-opens the note at that heading. The sheet's **★ Favorite** action is the exception: `DocumentViewModel.toggleIntroFavorite` pins the intro to the nav drawer keyed at `INTRO_LINE_INDEX` with no `:CUSTOM_ID:`, so favoriting a heading-less note never forces a blank heading onto the file.

  **Pending buffer.** The Read/Edit toggle writes nothing, so read mode has to render whatever the editor holds. The `note/{noteId}` route hoists the `EditorViewModel` (the mode toggle never leaves that back-stack entry, so a dirty buffer survives it) and hands read mode a `PendingEdit(fileName, lineIndex, text)`. `DocumentViewModel.setPendingEdit` stores it; `load` splices it over that subtree (`OrgMutations.replaceSubtree`) before parsing, so the rendered note is the unsaved text. Read mode's own mutations run through one choke point, `DocumentViewModel.saveDoc`: when the mutation left the subtree at `lineIndex` intact (a checkbox tap, a metadata-sheet edit) its new subtree text goes back into the editor's buffer via `EditorViewModel.onBufferChangedExternally` and the file is untouched; when it restructured the file instead (refile, delete, move, auto-archive) the computed text — which already carries the unsaved edits — is written as-is and `onBufferPersistedElsewhere` clears the editor's dirty flag so a later save can't clobber the result. Leaving the note for real (back gesture or the ← arrow, not the toggle) with a pending buffer raises the same `UnsavedNoteDialog` the editor uses, and read mode's top bar carries the editor's green save icon while one exists. Re-entering edit mode skips the reload from disk (`resumedDirty`) so the buffer is not thrown away.
...
- **"NEW" feature dots** (`ui/newbadge/`): a small config-driven framework for pointing returning users at a newly-shipped feature. Each feature is one `NewFeature(id, since, anchors, destination)` in the `NEW_FEATURES` registry (`NewFeatures.kt`); `since` is the `versionCode` it shipped in, `anchors` are string keys for every navigational element that should carry a dot, `destination` is the one whose viewing retires it. `NewBadgeState.from(GroveSettings)` derives the live set: a feature is active when a baseline is recorded, `since > baseline`, and the id isn't in `seenNewFeatures`. The baseline (`GroveSettings.newBadgeBaseline`, DataStore key `new_badge_baseline`) is seeded once — to `lastSeenChangelogBuild ?: currentBuild` — so a fresh install lands on the current build and shows nothing, while an update carries its older build and dots anything newer. State reaches the UI through `LocalNewBadges` (provided at `GroveNavigation`); a call site renders `NewDot(anchorKey)` (a 7dp accent dot inline beside a label, nothing when inactive) or wraps an icon glyph in `NewDotBadge(anchorKey) { … }` (the same dot corner-mounted top-right, ringed in `c.bg`) — a feature living inside a menu lists the container glyph's anchor (`TOPBAR_MENU`) so that glyph is dotted too. The destination site adds `MarkNewFeatureSeen(anchorKey)`, which writes every matching feature id into `seenNewFeatures` when that screen *leaves* composition (so a dot on the destination itself stays visible for the whole visit) — clearing the dot from the whole trail at once. `newBadgeBaseline` / `seenNewFeatures` are device-specific and excluded from settings export. Debug builds expose Settings › Developer › "Reset New badges" (`resetNewBadges()`: clears the seen set, drops the baseline to 0).
