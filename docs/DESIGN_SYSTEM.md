...
| Settings › Capture Templates | `settings/templates` | Reorderable template list, "＋ New template" row, capture-from-notification toggle. Template editor's Target file field is a free-text field with an inline expanding suggestion list (▾ trailing icon toggles it, typing filters it) below the field, `DropdownPicker`'s chrome (`surface2` fill, 13dp radius, scrollable past ~4 rows) rather than a `DropdownMenu` pop-out — a popup overlapped the field and stole the IME on each keystroke |
