...
| Settings › Capture Templates | `settings/templates` | Reorderable template list, "＋ New template" row, capture-from-notification toggle. Template editor's Target file field is a free-text field with a `DropdownMenu` pop-out (▾ trailing icon) listing the vault's existing notebooks, same pop-out-on-tap pattern as the widget quick-add's notebook chip — not a filter-as-you-type search |
